package com.myou.shardingsphere.service;

import com.myou.shardingsphere.model.dto.TAddressDto;
import com.myou.shardingsphere.model.vo.TAddressVo;
import com.github.pagehelper.*;

/**
 * 收货地址(TAddress)表服务接口
 *
 * @author makejava
 * @since 2021-07-21 15:36:07
 */
public interface TAddressService {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    TAddressVo findById(Long id);

    /**
     * 查询多条数据
     *
     * @param tAddressDto 实例对象
     * @param pageNum 页数
     * @param pageSize 每页条数
     * @return 对象列表
     */
    PageInfo<TAddressVo> findPageList(TAddressDto tAddressDto, int pageNum, int pageSize);

    /**
     * 新增数据
     *
     * @param tAddressDto 实例对象
     * @return 实例对象
     */
    TAddressVo add(TAddressDto tAddressDto);

    /**
     * 修改数据
     *
     * @param tAddressDto 实例对象
     * @return 实例对象
     */
    TAddressVo modify(TAddressDto tAddressDto);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 是否成功
     */
    boolean remove(Long id);

}